...
| 既存の音楽ファイルを取り込む | ツールバー「🎵 Add Files」(複数選択可) / 「📁 Add Folder」(フォルダを丸ごと・サブフォルダも再帰的に走査、取り込み済みはスキップ) / ウィンドウへファイル・フォルダをドラッグ&ドロップ |
...
- プレイリスト表示中は検索ボックス横のスコープ切り替えで「このプレイリスト」(既定) /
  「ライブラリ全体」を選べる
...
空白区切りのトークンは AND で結合される。`キー:値` はフィールド指定、それ以外は
タイトル/アーティスト/アルバム/アルバムアーティスト/ジャンル/コメントへの部分一致になる。
値に空白を含めたいときは `artist:"daft punk"` のように二重引用符で囲む。

| 書き方 | 意味 |
| --- | --- |
| `daft punk` | フリーテキスト (両方を含む) |
| `artist:daft` / `artist:"daft punk"` | アーティスト名の部分一致 |
| `album:discovery` / `albumartist:aphex` | アルバム / アルバムアーティスト |
| `genre:house` / `comment:classic` | ジャンル / コメント |
...
- スマートプレイリストの判定条件はアプリ内の SmartPlaylistEditor で編集できます。ただし iTunes XML の `Smart Info` / `Smart Criteria` は読み込み・書き出しともに保持しません (条件は Crateforge 側で保持します)。
- ライブラリ全体の置換のみ対応 (既存 XML への差分マージは未実装)。
- ドラッグ&ドロップでのプレイリスト追加とプレイリスト内の手動並べ替えに対応しています (#162)。右クリックメニューからも同じ操作ができます。
